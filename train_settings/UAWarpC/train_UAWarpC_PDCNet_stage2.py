...
def run(settings):
    settings.description = 'Default train settings for UAWarpCPDCNet, stage2'
    ...
    settings.initial_pretrained_model = os.path.join(settings.env.workspace_dir,
                                                     'train_settings/UAWarpC/train_UAWarpC_PDCNet_stage1/',
    ...
    # apply pre-processing to the images, here most of pre-processing is done within the function
    ...
    # sample an elastic flow, at the same settings.dataset_img_size than the images (outputted by dataloader)
